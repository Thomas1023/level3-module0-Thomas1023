...
for (int i = 0; i < sparks.length; i++) {
	...
	public void updateSparks() {
		...
		for(int i = 0; i < sparks.length; i++) {
			...
			if(s.y >= FireworkDisplay.HEIGHT||s.x >= FireworkDisplay.WIDTH||s.y <= 0||s.x <= 0) {
				Random r = new Random();
				s.xVelocity = r.nextInt(20) - s.xVelocity ;
				s.yVelocity = r.nextInt(20) - s.yVelocity;	
						}
		}
	...
	public void drawSparks(Graphics g) {
		...
		for (Spark spark : sparks) {
			...
			g.fillOval(spark.x, spark.y, 20, 20);
		}
